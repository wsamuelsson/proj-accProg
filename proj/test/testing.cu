...
int main(){
    ...
    crsMatrix<floatType>::sell_c_sigma_data_t sell_c_sigma_data = csrDelta.get_sell_c_sigma(32, 1);

    sellCSigmaMatrix<floatType> A_sellcsigma(sell_c_sigma_data);
    ...
    //GPU mult
    crsMatrix<floatType>::cugemv(csrDelta, x, y_sp);
    l2error = 0.0;
    for(int i = 0;i < N*N*N;i++){
        l2error += (y_sp[i] - y_dense[i])*(y_sp[i] - y_dense[i]);
    }
